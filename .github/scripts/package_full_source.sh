...
#   Outputs kristal-i18n-example-<tag>-full-source.tar.gz / .zip into $ROOT/dist
#   (override with KRISTAL_I18N_EXAMPLE_OUTPUT_DIR).

SCRIPT_DIR="$(CDPATH= cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P)"
ROOT="$(CDPATH= cd -- "$SCRIPT_DIR/../.." && pwd -P)"
TAG="${1:?usage: package_full_source.sh <tag>}"
OUTPUT_DIR="${KRISTAL_I18N_EXAMPLE_OUTPUT_DIR:-$ROOT/dist}"
mkdir -p "$OUTPUT_DIR"
...
tar "${TAR_EXCLUDES[@]}" -czf "$OUTPUT_DIR/kristal-i18n-example-$TAG-full-source.tar.gz" -C "$ROOT" .
test -s "$OUTPUT_DIR/kristal-i18n-example-$TAG-full-source.tar.gz"

# zip is optional but preferred for Windows users; -x patterns without a slash
# match any path component.
( cd "$ROOT" && zip -qr "$OUTPUT_DIR/kristal-i18n-example-$TAG-full-source.zip" . \
    -x '.git*' '.build*' 'dist*' '.tools*' '.worktrees*' )
test -s "$OUTPUT_DIR/kristal-i18n-example-$TAG-full-source.zip"

printf 'Created full source packages:\n  %s\n  %s\n' \
  "$OUTPUT_DIR/kristal-i18n-example-$TAG-full-source.tar.gz" \
  "$OUTPUT_DIR/kristal-i18n-example-$TAG-full-source.zip"
